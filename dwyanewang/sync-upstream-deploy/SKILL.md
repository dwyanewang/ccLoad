...
description: Synchronize ccLoad's local master and origin/master with the latest official upstream master, atomically rebuild rw-main from the Docker configuration and listed integration branches, then rebuild and verify Docker Compose. Use when asked to 一键更新上游并部署、同步本地与 origin、拉取 ccLoad 上游最新提交、重建 rw-main 后部署 Docker，或更新本地源码部署。
---

# 同步上游并部署

使用内置脚本从干净上游基线整体重建 `rw-main`；候选构建通过后，将本地 `master` 和 `origin/master` 快进到同一个官方上游提交，再交给项目外、长期运行的排空发布器。不要手工复制脚本中的 Git 步骤。

## 分支职责

- `master`：官方 `upstream/master` 的干净本地镜像，不存放本地改动。
- `origin/master`：官方 `upstream/master` 在个人远端的快进镜像，不存放独立提交。
...
4. 报告原/新 `master`、原/新 `origin/master`、上游目标、候选 commit、原/新 `rw-main`、备份分支、合并的清单分支和 Compose 健康状态。
...
- 默认使用 `upstream=https://github.com/caidaoli/ccLoad.git`；仅在远程不存在时添加，不覆盖已有远程。要求已有 `origin` 远程。
- 要求当前 `master` 和 `origin/master` 都可 fast-forward 到上游目标；任何一端分叉都停止，不修改配置分支历史。
- 从上游目标创建临时候选分支，依次 `--no-ff` 合并配置分支和清单分支。
- 在候选分支上完成 Compose 配置验证和镜像构建；任一步失败时删除候选，本地 `master`、远端 `origin/master` 和旧 `rw-main` 保持不变。
- 验证通过后，先用普通 push 将 `origin/master` 快进到锁定的上游目标；远端验证一致后，将旧 `rw-main` 保存为 `rw-main-backup-latest`，再原子更新本地 `master` 和 `rw-main`。
- 只允许向 `origin/master` 执行普通快进 push；永不执行 stash、hard reset、rebase、force push，也不 push 其他分支。
...
`--dry-run` 会 fetch 上游与 origin、创建和验证临时候选，但不 push、不移动本地 `master`/`rw-main` 且不启动服务。用户明确要求更新部署时直接执行正常模式。
...
- `CCLOAD_ORIGIN_REMOTE`：默认 `origin`
