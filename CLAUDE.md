...
| 渠道管理账户 | `channel_management_service.go`(核心 CAS 服务)→ `channel_management_new_api.go` / `channel_management_sub2api.go`(provider)→ `channel_management_scheduler.go`(每日签到)→ `admin_channel_management.go`(Admin API + 审计) |
...
- **Zed 原生登录契约**(`zedauth/`+`app/zed_wire.go`+`app/zed_credentials.go`+`app/admin_zed_auth.go`):`auth_type=zed_oauth`。这不是 OAuth code/PKCE：每次登录在随机 loopback 端口生成临时 RSA-2048 密钥，把 PKCS#1 DER 公钥以 base64url 传给 `zed.dev/native_app_signin`，回调的 `access_token` 用 RSA-OAEP/SHA-256 解密成长期 native credential；临时私钥绝不持久化。trial 权限绑定真实 Zed 安装的 `system_id`：按登录表单显式值 → `CCLOAD_ZED_SYSTEM_ID` → 本机 Zed `db/0-global/db.sqlite` 的 `kv_store.system_id` 取值；新账号没有可信来源就拒绝登录，禁止生成随机值或复制其他机器的固定值；同账号重授权必须保留已存值。数据请求先用 `Authorization: <user_id> <native-json>`+`x-zed-system-id` 调 `/client/llm_tokens` 换短期 JWT，再以 Bearer 调 `/completions`；JWT 提前 60 秒单飞刷新并 CAS 持久化，401 刷新必须绑定被拒绝的 JWT，迟到的旧 401 不得再次 mint。渠道固定 exact `/completions`、codex 协议+local 转换、禁用 WebSocket；动态暴露 `/models` 中 ccLoad 已实现 wire 转换的 `open_ai`、`anthropic`、`google` 模型。请求将标准 Responses body 按模型 provider 转换后包进 Zed `thread_id/prompt_id/intent/provider/model/provider_request` envelope，展开 Zed 不支持的 namespace 外壳并限定子工具名、合并 `additional_tools`、归一工具调用历史和 developer role/null 字段及 GPT-5.6 reasoning；OpenAI 响应恢复 namespace 工具身份，其他 provider 的逐行 `{"event":...}` 转换为标准 Responses SSE，收到结构化 `{"status":"stream_ended"}` 立即关闭上游；`plan` 403 只冷却当前模型并切换渠道，其他 401/403 才刷新凭证。额度用 native credential 读取 `/client/users/me`，快照写入 `oauth_usage`，不记 `quota_cost_usage`
