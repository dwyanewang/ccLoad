...
// ConvertOpenAIResponsesRequestToGemini converts a Responses request to Gemini.
func ConvertOpenAIResponsesRequestToGemini(modelName string, inputRawJSON []byte, stream bool) []byte {
						...
						case "input_image":
							...
							if imageURL != "" {
								...
								if data != "" {
									...
									partJSON = []byte(`{"fileData":{"fileUri":"","mimeType":"image/*"}}`)
									partJSON, _ = sjson.SetBytes(partJSON, "fileData.fileUri", imageURL)
								}
							}
						case "input_file":
							fileURI := contentItem.Get("file_url").String()
							if fileURI == "" {
								fileURI = contentItem.Get("file_id").String()
							}
							if fileURI != "" {
								partJSON = []byte(`{"fileData":{"fileUri":"","mimeType":"application/octet-stream"}}`)
								partJSON, _ = sjson.SetBytes(partJSON, "fileData.fileUri", fileURI)
							} else if data := contentItem.Get("file_data").String(); data != "" {
								partJSON = []byte(`{"inlineData":{"mimeType":"application/octet-stream","data":""}}`)
	...
	if contents := gjson.GetBytes(out, "contents"); len(contents.Array()) == 0 {
		if systemInstruction := gjson.GetBytes(out, "systemInstruction"); systemInstruction.Exists() {
			userContent := []byte(systemInstruction.Raw)
			userContent, _ = sjson.SetBytes(userContent, "role", "user")
