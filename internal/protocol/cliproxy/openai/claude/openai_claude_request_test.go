...
func TestConvertClaudeRequestToOpenAI_ThinkingToReasoningContent(t *testing.T) {
	tests := []struct {
		...
		{
			name: "AC1: unsigned assistant thinking is preserved",
			inputJSON: `{
				"model": "claude-3-opus",
				"messages": [{
					"role": "assistant",
		...
		{
			name: "AC3: unsigned thinking-only message is preserved",
			inputJSON: `{
				"model": "claude-3-opus",
				"messages": [{
					"role": "assistant",
		...
		{
			name: "Unsigned thinking parts are preserved",
			inputJSON: `{
				"model": "claude-3-opus",
				"messages": [{
					"role": "assistant",
...
// that unsigned Claude thinking remains available to OpenAI-compatible clients.
func TestConvertClaudeRequestToOpenAI_UnsignedThinkingOnlyMessagePreserved(t *testing.T) {
	...
	if len(messages) != 3 {
		t.Fatalf("Expected unsigned thinking-only assistant message to be preserved, got %d. Messages: %v", len(messages), resultJSON.Get("messages").Raw)
	}
	if got := messages[1].Get("role").String(); got != "assistant" {
		t.Fatalf("thinking-only message role = %q, want assistant", got)
	}
	if got := messages[1].Get("reasoning_content").String(); got != "Let me calculate: 2+2=4" {
		t.Fatalf("reasoning_content = %q, want preserved thinking. Messages: %v", got, resultJSON.Get("messages").Raw)
	}
	if content := messages[1].Get("content"); content.Exists() && content.String() != "" {
		t.Fatalf("thinking-only message unexpectedly has visible content: %s", messages[1].Raw)
	}
}
...
func TestConvertClaudeRequestToOpenAI_AssistantThinkingToolUseThinkingSplit(t *testing.T) {
	...
	// Unsigned thinking, text, and tool_calls remain unified.
	if len(messages) != 1 {
		t.Fatalf("Expected 1 message, got %d. Messages: %s", len(messages), resultJSON.Get("messages").Raw)
	}
