...
func convertOpenAIChatCompletionsResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	if isDone {
		...
		hasMessageOrTool := len(st.MsgItemAdded) > 0 || len(st.FuncItemAdded) > 0
		if !hasMessageOrTool && st.FinishReason == "" {
			return out
		}
...
func convertOpenAIChatCompletionsResponseToOpenAIResponsesNonStream(originalRequestRawJSON, requestRawJSON, rawJSON []byte) []byte {
	...
	if reasoning := message.Get("reasoning"); reasoning.IsArray() {
		for _, item := range reasoning.Array() {
			if rcText == "" {
				rcText = item.Get("text").String()
				if rcText == "" {
					for _, part := range item.Get("content").Array() {
						if part.Get("type").String() == "reasoning_text" {
							rcText += part.Get("text").String()
						}
					}
				}
			}
	...
	includeReasoning := rcText != "" || encryptedContent != ""
	...
	if includeReasoning {
		...
		reasoningItem := []byte(`{"id":"","type":"reasoning","summary":[],"content":[]}`)
		...
		if rcText != "" {
			...
			reasoningItem, _ = sjson.SetBytes(reasoningItem, "content.0.type", "reasoning_text")
			reasoningItem, _ = sjson.SetBytes(reasoningItem, "content.0.text", rcText)
		}
		if encryptedContent != "" {
	...
	if usage := root.Get("usage"); usage.Exists() {
		...
		if usage.Get("prompt_tokens").Exists() || usage.Get("completion_tokens").Exists() || usage.Get("total_tokens").Exists() {
			...
			if d := usage.Get("cache_creation_input_tokens"); d.Exists() {
				resp, _ = sjson.SetBytes(resp, "usage.cache_creation_input_tokens", d.Int())
			}
