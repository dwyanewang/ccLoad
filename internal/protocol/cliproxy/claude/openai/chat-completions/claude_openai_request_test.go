...
func TestConvertOpenAIRequestToClaude_SystemOnlyInputBecomesUserMessage(t *testing.T) {
	inputJSON := `{
		"model": "gpt-4.1",
		"messages": [
			{"role": "system", "content": "You are a helpful assistant."}
	...
	system := resultJSON.Get("system").Array()
	if len(system) != 0 {
		t.Fatalf("Expected system-only prompt to be moved into messages, got system: %s", resultJSON.Get("system").Raw)
	}

	messages := resultJSON.Get("messages").Array()
	if len(messages) != 1 {
		t.Fatalf("Expected 1 user message, got %d. Messages: %s", len(messages), resultJSON.Get("messages").Raw)
	}
	if got := messages[0].Get("role").String(); got != "user" {
		t.Fatalf("Expected message role %q, got %q", "user", got)
	}
	if got := messages[0].Get("content.0.type").String(); got != "text" {
		t.Fatalf("Expected content type %q, got %q", "text", got)
	}
	if got := messages[0].Get("content.0.text").String(); got != "You are a helpful assistant." {
		t.Fatalf("Expected user text %q, got %q", "You are a helpful assistant.", got)
	}
}
