...
// Expects top-level instructions + user msg + function_call + function_call_output.
...
func TestToolCallSimple(t *testing.T) {
	...
	if got := gjson.Get(result, "instructions").String(); got != "You are a helpful assistant." {
		t.Fatalf("instructions = %q, want %q", got, "You are a helpful assistant.")
	}
...
func TestCustomToolCallResponseFollowUpRoundTrip(t *testing.T) {
	...
	chatResponse := ConvertCodexResponseToOpenAINonStream(context.TODO(), "", originalRequest, nil, upstreamResponse, nil)
