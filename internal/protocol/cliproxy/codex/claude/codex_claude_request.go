...
func convertClaudeRequestToCodex(modelName string, inputRawJSON []byte, _ bool, preserveEmptyThinkingBlocks bool) []byte {
	...
	// Codex exposes top-level instructions; keep system text there instead of
	// synthesizing a developer message so system-only requests stay minimal.
	...
	if systemsResult.Exists() {
		...
		if len(instructions) > 0 {
			template, _ = sjson.SetBytes(template, "instructions", strings.Join(instructions, "\n"))
		}
	}
	...
	if messagesResult.IsArray() {
		...
		for i := 0; i < len(messageResults); i++ {
			...
			appendFileContent := func(fileID, fileData, filename string) {
				...
				if fileID != "" {
					content, _ = sjson.SetBytes(content, "file_id", fileID)
				} else {
					content, _ = sjson.SetBytes(content, "file_data", fileData)
				}
				if filename != "" {
			...
			if messageContentsResult.IsArray() {
				...
				for j := 0; j < len(messageContentResults); j++ {
					...
					case "redacted_thinking":
						...
						encryptedContent := messageContentResult.Get("data").String()
						if encryptedContent == "" {
							continue
						}
						flushMessage()
