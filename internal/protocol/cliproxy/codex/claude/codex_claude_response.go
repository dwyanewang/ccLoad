...
func ConvertCodexResponseToClaude(_ context.Context, modelName string, originalRequestRawJSON, _ []byte, rawJSON []byte, param *any) [][]byte {
	...
	var template []byte
	if typeStr != "" && typeStr != "error" && typeStr != "response.created" {
		output = append(output, startCodexMessage(params, rootResult)...)
	}
	...
	case "response.output_item.done":
		...
		case "reasoning":
			...
			if !params.ThinkingSummarySeen {
				itemResult.Get("summary").ForEach(func(_, summary gjson.Result) bool {
					if summary.Get("type").String() != "summary_text" {
						return true
					}
					text := summary.Get("text").String()
					if text == "" {
						return true
					}
					output = append(output, startCodexThinkingBlock(params)...)
					...
					delta, _ = sjson.SetBytes(delta, "delta.thinking", text)
...
func startCodexMessage(params *ConvertCodexResponseToClaudeParams, root gjson.Result) []byte {
	...
	template, _ = sjson.SetBytes(template, "message.model", params.Model)
	template, _ = sjson.SetBytes(template, "message.id", params.ResponseID)
	params.MessageStarted = true
	return translatorcommon.AppendSSEEventBytes(nil, "message_start", template, 2)
}
...
// ClaudeTokenCount builds an Anthropic token count response.
