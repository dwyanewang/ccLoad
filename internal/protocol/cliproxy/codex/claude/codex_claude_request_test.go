...
func TestConvertClaudeRequestToCodex_SystemMessageScenarios(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Message system role does not become top-level instructions",
			inputJSON: `{
				"model": "claude-3-opus",
				"messages": [
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, input := range resultJSON.Get("input").Array() {
				if input.Get("role").String() == "developer" {
					t.Fatalf("input[%d] unexpectedly contains a developer message: %s", i, input.Raw)
				}
			}
		})
	}
...
func TestConvertClaudeRequestToCodex_MessageSystemRoleWrapsAsUserReminder(t *testing.T) {
	...
	if got := gjson.GetBytes(result, "instructions").String(); got != "Top-level rules" {
		t.Fatalf("top-level instructions = %q, want %q", got, "Top-level rules")
	}
	if len(inputs) != 4 {
		t.Fatalf("got %d input items, want 4: %s", len(inputs), gjson.GetBytes(result, "input").Raw)
	}
	if got := inputs[1].Get("role").String(); got != "user" {
		t.Fatalf("message-level system role = %q, want user", got)
	}
	if got := inputs[1].Get("content.0.text").String(); got != "<system-reminder>\nFollow the project instructions\n</system-reminder>" {
		t.Fatalf("unexpected first reminder text: %q", got)
	}
	if got := inputs[3].Get("role").String(); got != "user" {
		t.Fatalf("array message-level system role = %q, want user", got)
	}
	if got := inputs[3].Get("content.0.text").String(); got != "<system-reminder>\nUse the current repo\n</system-reminder>" {
...
func TestConvertClaudeRequestToCodex_PreservesContentOrderAcrossToolAndReasoningItems(t *testing.T) {
	...
	result := ConvertClaudeRequestToCodex("gpt-5.4", []byte(inputJSON), false)
	if got := gjson.GetBytes(result, "instructions").String(); got != "system rules" {
		t.Fatalf("instructions = %q, want system rules. Output: %s", got, result)
	}
	inputs := gjson.GetBytes(result, "input").Array()
	if len(inputs) != 7 {
		t.Fatalf("got %d input items, want 7. Output: %s", len(inputs), result)
	}

	wantTypes := []string{"message", "reasoning", "message", "function_call", "message", "function_call_output", "message"}
	...
	if got := inputs[0].Get("content.0.text").String(); got != "before reasoning" {
		t.Fatalf("input[0] text = %q, want before reasoning", got)
	}
	if got := inputs[2].Get("content.0.text").String(); got != "before tool" {
		t.Fatalf("input[2] text = %q, want before tool", got)
	}
	if got := inputs[4].Get("content.0.text").String(); got != "after tool" {
		t.Fatalf("input[4] text = %q, want after tool", got)
	}
	if got := inputs[5].Get("output.0.type").String(); got != "input_text" {
		t.Fatalf("tool result output.0.type = %q, want input_text", got)
	}
	if got := inputs[5].Get("output.1.image_url").String(); got != "data:image/png;base64,aW1hZ2U=" {
		t.Fatalf("tool result image_url = %q, want data URL", got)
	}
	if got := inputs[6].Get("content.0.text").String(); got != "continue" {
		t.Fatalf("input[6] text = %q, want continue", got)
	}
}
